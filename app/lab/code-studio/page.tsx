  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 pt-28 sm:pt-36 pb-24">
      ...
      <div className="bg-[#0f1219] p-4 rounded-2xl border border-white/10 mb-6 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1 rounded-lg text-xs font-medium bg-white/5 hover:bg-white/10 text-neutral-300 border border-white/5 transition-all"
          >
            Glassmorphic Card
          </button>
          <button
        ...
        <button
          onClick={runCode}
          className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-cyan-400 hover:bg-cyan-300 text-neutral-900 font-semibold text-xs transition-all active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch mb-6">
        {/* Editor Pane */}
        <div className="bg-[#0f1219] p-4 rounded-3xl border border-white/10 flex flex-col h-[520px]">
          {/* Editor Tab Headers */}
            ...
            <div className="flex items-center gap-1 bg-black/40 p-1 rounded-xl border border-white/5 text-xs">
              <button
                ...
                className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                  activeTab === 'html' ? 'bg-cyan-500/20 text-cyan-400' : 'text-neutral-500 hover:text-neutral-300'
                }`}
              >
                HTML
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                  activeTab === 'css' ? 'bg-cyan-500/20 text-cyan-400' : 'text-neutral-500 hover:text-neutral-300'
                }`}
              >
                CSS
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                  activeTab === 'js' ? 'bg-cyan-500/20 text-cyan-400' : 'text-neutral-500 hover:text-neutral-300'
                }`}
              >
                JavaScript
              </button>
          ...
          <div className="flex-1 w-full relative">
            {activeTab === 'html' && (
              <textarea
                ...
                className="w-full h-full p-4 font-mono text-xs text-neutral-200 bg-black/60 border border-white/10 rounded-2xl resize-none focus:outline-none focus:border-cyan-400 transition-colors"
                spellCheck={false}
              />
            )}
            {activeTab === 'css' && (
      ...
      <div className="bg-[#0f1219] p-4 rounded-2xl border border-white/10 space-y-2">
        <div className="flex items-center justify-between text-xs pb-2 border-b border-white/5">
          <span className="font-semibold text-neutral-300 flex items-center gap-1.5">
            <Terminal className="w-3.5 h-3.5 text-cyan-400" />
            <span>Console Output</span>
          </span>
          <button onClick={() => setLogs([])} className="text-neutral-500 hover:text-neutral-300 text-[11px]">
            Clear Logs
          </button>
        </div>
        ...
        <div className="font-mono text-xs text-neutral-300 bg-black/40 p-3 rounded-xl max-h-32 overflow-y-auto space-y-1">
          {logs.length > 0 ? (
            ...
            <span className="text-neutral-600">No console output recorded yet.</span>
          )}
        </div>
      </div>
    </div>
