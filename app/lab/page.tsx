...
export default function LabHubPage() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 pt-28 sm:pt-36 pb-24">
      ...
      <div className="bg-[#0f1219] p-4 rounded-2xl border border-white/8 mb-8 space-y-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`inline-flex items-center px-3.5 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-all ${activeCategory === cat.id ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-[#0f1219] text-neutral-400 hover:text-neutral-200 border border-white/8'}`}>
              {cat.name}
            </button>
          ))}
        </div>
      ...
      {filteredTools.length === 0 && (
        <div className="bg-[#0f1219] p-12 rounded-2xl text-center space-y-3 max-w-md mx-auto my-12 border border-white/8">
