  ...
  return (
    <section id="about" className="py-16 sm:py-20 px-4 sm:px-6 max-w-6xl mx-auto scroll-mt-24">
      <div className="flex flex-col md:flex-row gap-10 items-start">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="glass-panel-elevated w-full md:w-5/12 p-6 sm:p-8 rounded-3xl relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="w-full md:w-7/12 space-y-6">
          ...
          <div className="space-y-4">
            {philosophies.map((item) => <div key={item.title} className="glass-panel p-5 rounded-2xl flex gap-4 items-start hover-surface"><div className="w-6 h-6 rounded-lg bg-white/[0.06] border border-white/10 text-white flex items-center justify-center shrink-0 mt-0.5"><CheckCircle2 className="w-4 h-4" /></div><div><h4 className="text-sm font-semibold text-white mb-1">{item.title}</h4><p className="text-xs text-neutral-400 leading-relaxed">{item.desc}</p></div></div>)}
          </div>
        </motion.div>
      </div>
    </section>
