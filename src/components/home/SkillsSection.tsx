...
export default function SkillsSection() {
  ...
  return <section className="py-16 sm:py-20 px-4 sm:px-6 max-w-6xl mx-auto">
    ...
    <div className="flex items-center justify-center gap-2 flex-wrap mb-8">{SKILL_CATEGORIES.map((cat) => <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all ${activeCategory === cat.id ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/30 shadow-cyan-500/10' : 'glass-panel text-neutral-400 hover:text-white border-white/5 hover:border-white/15'}`}>{icons[cat.iconName]}<span>{cat.name}</span></button>)}</div>
