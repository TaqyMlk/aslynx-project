...
export default function SkillsSection() {
  ...
  return (
    <section className="py-20 sm:py-28 px-4 sm:px-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-center gap-2 flex-wrap mb-8">{SKILL_CATEGORIES.map((cat) => <button key={cat.id} onClick={() => setActiveCategory(cat.id)} aria-pressed={activeCategory === cat.id} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all ${activeCategory === cat.id ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-[#020617] text-neutral-400 hover:text-white border border-white/8 hover:border-white/15'}`}>{icons[cat.iconName]}<span>{cat.name}</span></button>)}</div>
      <motion.div key={`${activeCategory}-${currentCategory.id}`} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.25, 0.1, 0.25, 1] as [number, number, number, number] }} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {currentCategory.skills.map((skill, idx) => (
          <motion.div key={`${activeCategory}-${skill.name}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: idx * 0.06, ease: [0.25, 0.1, 0.25, 1] as [number, number, number, number] }} className={`p-5 rounded-2xl flex flex-col gap-2 border ${skill.highlight ? 'bg-[#020617] border-white/12' : 'bg-[#020617] border-white/8'}`}>
