    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-3 py-3 sm:px-4 sm:py-4">
        <nav
          ...
          className={`w-full max-w-5xl flex items-center justify-between px-3.5 sm:px-5 py-2.5 rounded-2xl border transition-colors duration-200 ${
            scrolled ? 'bg-[#11131a]/96 border-white/10 shadow-[0_12px_30px_-24px_rgba(0,0,0,.9)]' : 'bg-[#0e1017]/94 border-white/[0.07]'
          }`}
        >
          <Link href="/" className="flex items-center gap-2.5 group min-w-0">
            <div className="w-8 h-8 rounded-xl bg-[#171a22] border border-white/10 flex items-center justify-center shrink-0 transition-colors group-hover:border-cyan-400/30">
              <span className="font-semibold text-[11px] text-cyan-300">AL</span>
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="font-semibold text-sm sm:text-base tracking-tight text-white group-hover:text-cyan-200 transition-colors">AsLynx</span>
                <span aria-label="Online" className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
              </div>
              <span className="text-[11px] text-zinc-500 hidden sm:block">AI & Bedrock systems</span>
            </div>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`relative px-3.5 py-1.5 text-sm font-medium rounded-lg transition-colors ${isActive ? 'text-white' : 'text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.04]'}`}
                >
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 text-sm font-medium text-zinc-300 bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.08] hover:border-white/[0.14] px-3 py-1.5 rounded-lg transition-colors active:bg-white/[0.1]"
          >
      ...
      {searchOpen && (
        <div className="fixed inset-0 z-[100] flex items-start justify-center px-3 sm:px-4 pt-[10vh] sm:pt-[14vh]">
          ...
          <motion.div
            ...
            className="relative w-full max-w-xl bg-[#14171f] border border-white/12 rounded-2xl shadow-[0_24px_70px_-28px_rgba(0,0,0,.95)] overflow-hidden"
          >
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/[0.07]">
              <Search className="w-4.5 h-4.5 text-cyan-300 shrink-0" />
              <input
                autoFocus
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Search projects, tools, pages..."
                aria-label="Search site"
                className="flex-1 min-w-0 bg-transparent outline-none text-sm text-white placeholder:text-zinc-600"
              />
              <button type="button" onClick={closeSearch} aria-label="Close search" className="p-1.5 rounded-lg text-zinc-500 hover:text-white hover:bg-white/[0.06] transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
                ...
                return internal ? (
                  ...
                  >
                    <div className="w-8 h-8 rounded-lg bg-white/[0.035] border border-white/[0.06] flex items-center justify-center shrink-0">
                      {result.href.startsWith('/lab') ? <Wrench className="w-3.5 h-3.5 text-zinc-500" /> : <ArrowUpRight className="w-3.5 h-3.5 text-zinc-500" />}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-medium text-zinc-100 truncate">{result.title}</div>
                      <div className="text-xs text-zinc-600 mt-0.5 truncate">{result.description}</div>
                    </div>
                    {isHash && <span className="text-[10px] text-zinc-700">section</span>}
                  </Link>
                ) : null;
              }) : (
                <div className="py-12 text-center">
                  <Search className="w-5 h-5 mx-auto text-zinc-700" />
                  <p className="mt-2 text-sm text-zinc-500">No matching pages or tools.</p>
                </div>
              )}
            </div>

            <div className="px-4 py-2.5 border-t border-white/[0.07] text-[11px] text-zinc-600 flex items-center justify-between">
              <span>Pages, tools and current-page sections</span>
              <span className="hidden sm:inline">Esc to close · Ctrl K</span>
            </div>
          </motion.div>
        </div>
      )}
