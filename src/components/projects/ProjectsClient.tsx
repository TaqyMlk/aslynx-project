  ...
  return (
    <main className="w-full max-w-6xl mx-auto px-4 sm:px-6 pt-28 sm:pt-36 pb-20">
      <header className="max-w-3xl mb-10">
        ...
        <p className="text-sm sm:text-base text-zinc-400 leading-relaxed max-w-2xl">Minecraft Bedrock projects, developer tools, and web applications built and maintained by AsLynx.</p>
      </header>
      ...
      <section aria-label="Project filters" className="border-y border-white/[0.08] py-4 mb-8">
        <div className="flex flex-col sm:flex-row gap-3">
          <label className="relative flex-1">
            ...
            <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search projects, technologies, or keywords" className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-transparent border border-white/[0.1] text-sm text-white placeholder:text-zinc-600 focus:border-cyan-400/60 focus:outline-none" />
          </label>
          <label className="flex items-center gap-2 sm:w-64">
            <SlidersHorizontal className="w-4 h-4 text-zinc-500 shrink-0" />
            <span className="sr-only">Sort projects</span>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as 'downloads' | 'name' | 'featured')} aria-label="Sort projects" className="w-full px-3 py-2.5 rounded-lg bg-transparent border border-white/[0.1] text-sm text-zinc-300 focus:border-cyan-400/60 focus:outline-none">
        ...
        <div className="flex items-center gap-5 overflow-x-auto pt-4 pb-1">
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} aria-pressed={selectedCategory === cat.id} className={`shrink-0 text-sm transition-colors ${selectedCategory === cat.id ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>
              {cat.name}
            </button>
          ))}
        </div>
      ...
      {filteredProjects.length > 0 ? (
        ...
        <div className="border-y border-white/[0.08] py-16 text-center max-w-lg mx-auto">
          <Box className="w-8 h-8 text-zinc-600 mx-auto mb-4" />
          <h2 className="text-base font-semibold text-white mb-2">No matching projects</h2>
          <p className="text-sm text-zinc-500 mb-5">Try another search or category.</p>
